...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        try {
            ...
            // Get dogs linked to this guardian
    ...
    if (req.method === 'PUT') {
        try {
            const allowedFields = [
                ...
                'last_checkin', 'checkin_frequency_days', 'checkin_notes',
                'status', 'notes'
            ];
        ...
        try {
            ...
            if (assign_dog_ids && assign_dog_ids.length > 0) {
                ...
                if (assignError) {
                    ...
                    return res.status(500).json({ error: 'Failed to assign dogs' });
                }
            ...
            if (unassign_dog_ids && unassign_dog_ids.length > 0) {
                ...
                if (unassignError) {
                    ...
                    return res.status(500).json({ error: 'Failed to unassign dogs' });
                }
