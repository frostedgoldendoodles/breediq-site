...
import { computeDueDate, gestationProgress, GESTATION_DAYS } from '../gestation.js';
...
export const TOOL_SCHEMAS = [
    ...
    {
        name: 'delete_dog',
        description: 'Propose permanently deleting a dog. Destructive: calling this never deletes anything on its own — it returns the record for the user to confirm, and the deletion only happens after they approve it in the UI. Call it whenever the user asks to delete a dog.',
        input_schema: {
            type: 'object',
            properties: {
                id: { type: 'string' },
    ...
    {
        name: 'delete_litter',
        description: 'Propose permanently deleting a litter. Destructive: calling this never deletes anything on its own — it returns the record for the user to confirm, and the deletion only happens after they approve it in the UI.',
        input_schema: {
            type: 'object',
            properties: {
                id: { type: 'string' },
    ...
    },
    {
        ...
        description: 'Propose permanently deleting a guardian. Destructive: calling this never deletes anything on its own — it returns the record for the user to confirm, and the deletion only happens after they approve it in the UI.',
        input_schema: {
            type: 'object',
            properties: {
                id: { type: 'string' },
    ...
    },
    {
        ...
        description: 'One-call snapshot of the whole breeding program: dog counts, active pregnancies with gestation day and due date, projected heat windows in the next 30 days, upcoming litter milestones (ultrasound, x-ray, due, go-home), overdue vet visits, and guardian totals. Call this FIRST for questions like "what needs attention", "how is my program doing", "what is coming up", or any daily-summary request — it replaces a chain of individual searches.',
        input_schema: {
            type: 'object',
            properties: {
                horizon_days: { type: 'number', description: 'How far ahead to look for upcoming events. Default 30, max 120.' }
            }
        }
    },
    {
...
// Tools that never execute on the model's say-so. The route resolves the
// `confirmed` flag from a structured confirmation the *client* sends (see
// api/assistant/chat.js); it is deliberately absent from the tool schemas so
// the model cannot assert it, and `input` is never consulted for it. Without
// that separation, any text the assistant reads — a dog's notes field, a
// guardian's address — could talk it into deleting records.
export const DESTRUCTIVE_TOOLS = new Set(['delete_dog', 'delete_litter', 'delete_guardian']);

// Does the client's approval authorize *this* destructive call?
//
// `confirm` is { tool_name, target_id } and comes from the request body, set
// by the Confirm button from the target the server resolved on the preceding
// requires_confirmation result. `input` is the model's tool input. The
// approval must name the same tool and the same record; a bare "yes" in the
// conversation, or the model re-calling with a different id, does not count.
export function isDestructiveCallConfirmed(confirm, toolName, input) {
    if (!DESTRUCTIVE_TOOLS.has(toolName)) return false;
    if (!confirm || typeof confirm !== 'object') return false;
    if (confirm.tool_name !== toolName) return false;
    const approved = confirm.target_id;
    if (typeof approved !== 'string' || approved === '') return false;
    return approved === input?.id;
}
...
// Resolve the program-owner's effective user_id list: themselves + any
// active sub-breeders linked via breeder_relationships. Reads & writes
// scope to this list so the assistant can manage records across the
// whole program (matching the dashboard's combined view).
async function getEffectiveUserIds(supabase, ownerUserId) {
    const { data } = await supabase
        .from('breeder_relationships')
        .select('breeder_id')
        .eq('owner_id', ownerUserId)
        .eq('status', 'active');
    return [ownerUserId, ...(data || []).map(r => r.breeder_id)];
}

// Strip PostgREST filter metacharacters from values interpolated into
// `.or()` / `.ilike()` strings. Tenant isolation does not rely on this (every
// query is AND-combined with a user-scope filter), but it prevents commas/
// parens/dots in a dog name or search term from corrupting the filter.
function safeFilter(value) {
    if (value === null || value === undefined) return '';
    return String(value).replace(/[,()*:.\\%]/g, ' ').trim().slice(0, 100);
}

async function findDogByName(supabase, userIds, name) {
    if (!name) return null;
    const safe = safeFilter(name);
    if (!safe) return null;
    const { data } = await supabase
        .from('dogs')
        .select('id, user_id, name, call_name, sex, status, role, heat_status')
        .in('user_id', userIds)
        .or(`name.ilike.${safe},call_name.ilike.${safe}`)
        .limit(5);
...
async function findLitter(supabase, userIds, { id, damNameMatch }) {
    if (id) {
        const { data } = await supabase
            .from('litters')
    ...
    if (damNameMatch) {
        const dam = await findDogByName(supabase, userIds, damNameMatch);
        if (!dam) return null;
        const { data } = await supabase
            .from('litters')
            .select('*, dam:dogs!litters_dam_id_fkey(id, name)')
...
async function findGuardian(supabase, userIds, { id, familyNameMatch }) {
    if (id) {
        const { data } = await supabase.from('guardians').select('*').in('user_id', userIds).eq('id', id).maybeSingle();
        return data || null;
    }
    if (familyNameMatch) {
        const { data } = await supabase
...
export async function executeTool(name, { user_id, supabase, input, confirm_delete }) {
    try {
        // Compute effective user_ids once per tool invocation so reads & writes
        // can scope across the program owner + active sub-breeders.
        const user_ids = await getEffectiveUserIds(supabase, user_id);
        const ctx = { user_id, user_ids, supabase, input, confirm_delete };
        switch (name) {
            case 'create_dog': return await createDog(ctx);
            case 'update_dog': return await updateDog(ctx);
            case 'delete_dog': return await deleteDog(ctx);
            case 'create_litter': return await createLitter(ctx);
            case 'update_litter': return await updateLitter(ctx);
            case 'delete_litter': return await deleteLitter(ctx);
            case 'create_guardian': return await createGuardian(ctx);
            case 'update_guardian': return await updateGuardian(ctx);
            case 'delete_guardian': return await deleteGuardian(ctx);
            case 'link_guardian_to_dog': return await linkGuardianToDog(ctx);
            case 'log_heat': return await logHeat(ctx);
            case 'create_calendar_event': return await createCalendarEvent(ctx);
            case 'get_program_overview': return await getProgramOverview(ctx);
            case 'search_records': return await searchRecords(ctx);
            case 'get_dog': return await getDog(ctx);
            case 'get_litter': return await getLitter(ctx);
            case 'get_guardian': return await getGuardian(ctx);
...
// update_dog
async function updateDog({ user_id, user_ids, supabase, input }) {
    let dogId = input.id;
    if (!dogId && input.name_match) {
        const hit = await findDogByName(supabase, user_ids, input.name_match);
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error) return err(error.message);
    if (!data) return err('Dog not found or not in your program');
    return ok({ dog: data, fields_updated: Object.keys(updates).filter(k => k !== 'updated_at') });
}

// delete_dog
async function deleteDog({ user_id, user_ids, supabase, input, confirm_delete }) {
    const confirmed = confirm_delete === true;
    let dogId = input.id;
    if (!dogId && input.name_match) {
        const hit = await findDogByName(supabase, user_ids, input.name_match);
        if (!hit) return err(`No dog found matching "${input.name_match}"`);
    ...
    if (!confirmed) {
        ...
        const { data } = await supabase.from('dogs').select('id, name, sex, status').eq('id', dogId).in('user_id', user_ids).single();
        return { ok: false, requires_confirmation: true, action: 'delete_dog', target: data };
    }
    const { error } = await supabase.from('dogs').delete().eq('id', dogId).in('user_id', user_ids);
    if (error) return err(error.message);
    return ok({ deleted: true, id: dogId });
}

// create_litter
async function createLitter({ user_id, user_ids, supabase, input }) {
    let damId = input.dam_id;
    let damOwnerId = user_id;
    if (!damId && input.dam_name) {
        const dam = await findDogByName(supabase, user_ids, input.dam_name);
        if (!dam) return err(`Dam "${input.dam_name}" not found`);
        damId = dam.id;
        damOwnerId = dam.user_id || user_id;
    } else if (damId) {
        // Resolve the dam's owning user_id so the litter is filed under the dam's account
        const { data: dam } = await supabase.from('dogs').select('id, user_id').eq('id', damId).in('user_id', user_ids).maybeSingle();
        if (dam?.user_id) damOwnerId = dam.user_id;
    }
    let sireId = input.sire_id;
    if (!sireId && input.sire_name) {
        const sire = await findDogByName(supabase, user_ids, input.sire_name);
        if (sire) sireId = sire.id; // sire may be an outside stud; it's ok to leave null
    }

    let due = input.due_date;
    if (input.breed_date && !due) {
        due = computeDueDate(input.breed_date);
    }
    ...
    if (damId && input.breed_date) {
        ...
        }).eq('id', damId).in('user_id', user_ids);
    }
...
// update_litter
async function updateLitter({ user_id, user_ids, supabase, input }) {
    const litter = await findLitter(supabase, user_ids, { id: input.id, damNameMatch: input.dam_name_match });
    ...
    if (updates.status && litter.dam_id) {
        ...
        if (mapped) {
            await supabase.from('dogs').update({
                heat_status: mapped,
                updated_at: new Date().toISOString()
            }).eq('id', litter.dam_id).in('user_id', user_ids);
        }
    }
...
// delete_litter
async function deleteLitter({ user_id, user_ids, supabase, input, confirm_delete }) {
    const confirmed = confirm_delete === true;
    const litter = await findLitter(supabase, user_ids, { id: input.id, damNameMatch: input.dam_name_match });
    if (!litter) return err('Litter not found');
    if (!confirmed) {
    ...
    const { error } = await supabase.from('litters').delete().eq('id', litter.id).in('user_id', user_ids);
    if (error) return err(error.message);
...
// update_guardian
async function updateGuardian({ user_id, user_ids, supabase, input }) {
    let guardian;
    if (input.id) guardian = await findGuardian(supabase, user_ids, { id: input.id });
    else if (input.family_name_match) guardian = await findGuardian(supabase, user_ids, { familyNameMatch: input.family_name_match });
...
// delete_guardian
async function deleteGuardian({ user_id, user_ids, supabase, input, confirm_delete }) {
    const confirmed = confirm_delete === true;
    let guardian;
    if (input.id) guardian = await findGuardian(supabase, user_ids, { id: input.id });
    else if (input.family_name_match) guardian = await findGuardian(supabase, user_ids, { familyNameMatch: input.family_name_match });
    ...
    const { error } = await supabase.from('guardians').delete().eq('id', guardian.id).in('user_id', user_ids);
    if (error) return err(error.message);
    return ok({ deleted: true, id: guardian.id });
}

// link_guardian_to_dog
async function linkGuardianToDog({ user_id, user_ids, supabase, input }) {
    let dogId = input.dog_id;
    if (!dogId && input.dog_name_match) {
        const dog = await findDogByName(supabase, user_ids, input.dog_name_match);
    ...
    if (!guardianId && input.guardian_family_match) {
        const g = await findGuardian(supabase, user_ids, { familyNameMatch: input.guardian_family_match });
...
// log_heat
async function logHeat({ user_id, user_ids, supabase, input }) {
    let dogId = input.dog_id;
    if (!dogId && input.dog_name_match) {
        const dog = await findDogByName(supabase, user_ids, input.dog_name_match);
        if (!dog) return err(`Dog "${input.dog_name_match}" not found`);
...
async function createCalendarEvent({ user_id, user_ids, supabase, input }) {
...
// search_records — searches across owner + active sub-breeders
// get_program_overview — the assistant's situational-awareness tool.
// Two queries (dogs + litters), everything else computed in-process, so it is
// cheap enough for the model to call at the start of any status conversation.
async function getProgramOverview({ user_ids, supabase, input }) {
    const DAY_MS = 24 * 60 * 60 * 1000;
    const horizon = Math.min(Math.max(Number(input?.horizon_days) || 30, 1), 120);
    const now = Date.now();
    const horizonEnd = now + horizon * DAY_MS;
    const iso = (t) => new Date(t).toISOString().slice(0, 10);

    const [dogsRes, littersRes] = await Promise.all([
        supabase.from('dogs')
            .select('id, name, sex, status, role, heat_status, last_heat_date, avg_heat_cycle_days, vet_last_visit, guardian_id')
            .in('user_id', user_ids),
        supabase.from('litters')
            .select('id, status, breed_date, due_date, whelp_date, ultrasound_date, xray_date, go_home_date, puppy_count, dam:dogs!litters_dam_id_fkey(id, name)')
            .in('user_id', user_ids)
            .not('status', 'eq', 'archived')
    ]);
    if (dogsRes.error) return err(dogsRes.error.message);
    if (littersRes.error) return err(littersRes.error.message);
    const dogs = dogsRes.data || [];
    const litters = littersRes.data || [];

    const active = dogs.filter(d => d.status === 'active' || d.status === 'guardian');
    const counts = {
        total_dogs: dogs.length,
        active_dogs: active.length,
        dams: active.filter(d => d.role === 'dam').length,
        studs: active.filter(d => d.role === 'stud').length,
        guardian_homed: dogs.filter(d => d.status === 'guardian' || d.guardian_id).length,
        in_heat: dogs.filter(d => d.heat_status === 'in_heat').length,
        pregnant: dogs.filter(d => d.heat_status === 'pregnant').length,
        nursing: dogs.filter(d => d.heat_status === 'nursing').length
    };

    // Active pregnancies with computed gestation position.
    const pregnancies = litters
        .filter(l => l.breed_date && !l.whelp_date && ['confirmed', 'planned'].includes(l.status))
        .map(l => {
            const prog = gestationProgress(l.breed_date);
            return {
                litter_id: l.id,
                dam: l.dam?.name || 'Unknown dam',
                status: l.status,
                gestation_day: prog.computed_gestation_day,
                of_days: GESTATION_DAYS,
                due_date: l.due_date || computeDueDate(l.breed_date),
                days_remaining: prog.days_remaining
            };
        })
        .sort((a, b) => a.days_remaining - b.days_remaining);

    // Projected heat windows inside the horizon.
    const heats_due = [];
    for (const d of dogs) {
        if (!d.last_heat_date || !d.avg_heat_cycle_days) continue;
        if (['pregnant', 'nursing', 'bred'].includes(d.heat_status)) continue;
        const last = new Date(d.last_heat_date).getTime();
        if (isNaN(last)) continue;
        let projected = last + d.avg_heat_cycle_days * DAY_MS;
        let guard = 0;
        while (projected < now && guard++ < 40) projected += d.avg_heat_cycle_days * DAY_MS;
        if (projected <= horizonEnd) {
            heats_due.push({ dog_id: d.id, dog: d.name, projected_date: iso(projected), overdue: projected < now });
        }
    }
    heats_due.sort((a, b) => a.projected_date.localeCompare(b.projected_date));

    // Litter milestones inside the horizon.
    const MILESTONES = [
        ['ultrasound_date', 'ultrasound'], ['xray_date', 'x-ray'],
        ['due_date', 'due date'], ['go_home_date', 'go-home day']
    ];
    const upcoming_milestones = [];
    for (const l of litters) {
        for (const [key, label] of MILESTONES) {
            if (!l[key]) continue;
            const t = new Date(l[key]).getTime();
            if (isNaN(t) || t < now - DAY_MS || t > horizonEnd) continue;
            upcoming_milestones.push({ litter_id: l.id, dam: l.dam?.name || 'Unknown dam', event: label, date: iso(t) });
        }
    }
    upcoming_milestones.sort((a, b) => a.date.localeCompare(b.date));

    // Vet visits more than a year stale.
    const yearAgo = now - 365 * DAY_MS;
    const vet_overdue = dogs
        .filter(d => d.vet_last_visit && new Date(d.vet_last_visit).getTime() < yearAgo && d.status === 'active')
        .map(d => ({ dog_id: d.id, dog: d.name, last_visit: d.vet_last_visit }));
    ...
    return ok({
        ...
        heats_due_within_horizon: heats_due,
        upcoming_litter_milestones: upcoming_milestones,
        vet_visits_overdue: vet_overdue
    });
}

async function searchRecords({ user_id, user_ids, supabase, input }) {
    const q = (input.query || '').trim();
    if (!q) return err('query is required');
    const type = input.entity_type || 'any';
    // Sanitize before wrapping in % wildcards so commas/parens in the model's
    // query string can't restructure the .or() filter group.
    const safe = safeFilter(q);
    if (!safe) return ok({ query: q, dogs: [], litters: [], guardians: [], summary: 'Found 0 dog(s), 0 litter(s), 0 guardian(s).' });
    const pattern = `%${safe}%`;
...
// get_dog
async function getDog({ user_id, user_ids, supabase, input }) {
    if (input.id) {
        const { data, error } = await supabase.from('dogs').select('*').eq('id', input.id).in('user_id', user_ids).maybeSingle();
        if (error) return err(error.message);
        if (!data) return err('Dog not found');
        return ok({ dog: data });
    }
    if (input.name) {
        const dog = await findDogByName(supabase, user_ids, input.name);
        if (!dog) return err(`No dog matching "${input.name}"`);
        const { data } = await supabase.from('dogs').select('*').eq('id', dog.id).in('user_id', user_ids).single();
        return ok({ dog: data });
    }
...
// get_litter
async function getLitter({ user_id, user_ids, supabase, input }) {
    const litter = await findLitter(supabase, user_ids, { id: input.id, damNameMatch: input.dam_name });
    if (!litter) return err('Litter not found');
    return ok({ litter });
}

// get_guardian
async function getGuardian({ user_id, user_ids, supabase, input }) {
    const guardian = await findGuardian(supabase, user_ids, { id: input.id, familyNameMatch: input.family_name });
    if (!guardian) return err('Guardian not found');
    return ok({ guardian });
}
